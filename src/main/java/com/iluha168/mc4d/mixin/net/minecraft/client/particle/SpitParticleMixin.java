...
@Mixin(SpitParticle.class)
abstract class SpitParticleMixin extends ExplodeParticleMixin {
	@Mixin(SpitParticle.Provider.class)
	static abstract class ProviderMixin implements ParticleProvider4<SimpleParticleType> {
		...
		public @Nullable Particle createParticle(SimpleParticleType options, ClientLevel level, double x, double y, double z, double w, double xAux, double yAux, double zAux, double wAux, RandomSource random) {
			final Particle particle = new SpitParticle(level, x, y, z, xAux, yAux, zAux, this.sprites);
