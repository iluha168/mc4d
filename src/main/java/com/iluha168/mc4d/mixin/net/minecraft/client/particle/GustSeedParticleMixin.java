...
abstract class GustSeedParticleMixin extends ParticleMixin {
	@Shadow
	@Final
	...
	void tick(ClientLevel instance, ParticleOptions particle, double x, double y, double z, double xd, double yd, double zd) {
		final double w = this.w() + (this.random.nextDouble() - this.random.nextDouble()) * this.scale;
		((LevelAccessor4) instance).addParticle(particle, x, y, z, w, xd, yd, zd, zd);
	}

	@Mixin(GustSeedParticle.Provider.class)
